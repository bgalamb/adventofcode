    ...
    for e in pathmap[0]:
        if e == "|":
            print("First {}".format((0,indx)))
                   # x, y, value, heading
            return (0,indx,"down",pathmap[0][indx],1) #down, up, left, right
        indx = indx + 1

def getNextElemDown(coordinate):
    #print("nextElemDown")
    if len(pathmap[coordinate[0]+1]) < coordinate[1]+1 :
        return (coordinate[0]+1,coordinate[1],"down"," ",coordinate[4])
    return (coordinate[0]+1,coordinate[1],"down",pathmap[coordinate[0]+1][coordinate[1]],coordinate[4]+1)

def getNextElemUp(coordinate):
    #print("nextElemUp")
    if len(pathmap[coordinate[0]-1]) < coordinate[1]+1:
        return (coordinate[0]-1,coordinate[1],"up"," ")
    return (coordinate[0]-1,coordinate[1],"up",pathmap[coordinate[0]-1][coordinate[1]],coordinate[4]+1)

def getNextElemLeft(coordinate):
    #print("nextElemLeft")
    if len(pathmap[coordinate[0]]) < coordinate[1]+1-1:
        return (coordinate[0],coordinate[1],"left"," ")
    return (coordinate[0],coordinate[1]-1,"left",pathmap[coordinate[0]][coordinate[1]-1],coordinate[4]+1)

def getNextElemRight(coordinate):
    #print("nextElemRight")
    if len(pathmap[coordinate[0]]) < coordinate[1]+1+1:
        return (coordinate[0],coordinate[1],"right"," ")
    return (coordinate[0],coordinate[1]+1,"right",pathmap[coordinate[0]][coordinate[1]+1],coordinate[4]+1)
...
print("Total steps {}".format(totalPlus))
